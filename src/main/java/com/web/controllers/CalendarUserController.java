...
public class CalendarUserController {
    ...
    public ModelAndView displayCalendar(@RequestParam Long idDoctor,
                                        @SessionAttribute UserForm userSession) {
        ...
        Set<CalendarTicketForm> calendars = doctorFacade.getCalendar(idDoctor);

        listCalendarUser.forEach(calendar -> calendars.removeIf(calendar::equals));

        List<CalendarTicketForm> collect = calendars.stream().filter(calendar ->
